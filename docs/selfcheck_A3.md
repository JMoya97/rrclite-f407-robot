...
- ACK coverage: On success, the handler builds `rrc_steer_ack_t` and calls `rrc_send_ack(RRC_FUNC_STEER, RRC_STEER_ACK, …)` (lines 798–805), echoing txid and the number of accepted targets. The validator row for sub 0x03 lives in `Core/Src/rrclite_packets.c` (lines 60–83).
