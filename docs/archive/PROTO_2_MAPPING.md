# Protocol 2 Mapping (Historical)

This note captures the migration from the legacy function identifiers to the
stable Protocol&nbsp;2 surface for reference when working with older host
software releases.

## Function ID Allocation

| Function | Value | Legacy coverage |
|----------|-------|-----------------|
...
- `0x12` → `STREAM_ACK`
- `0x20` → `ONE_SHOT`
- IMU configuration historically reserved `0x31` (preset) and `0x32` (biases).

## Legacy (v1) → Protocol 2 Translation Table

| Legacy func/sub | Protocol 2 func/sub | Notes |
|-----------------|---------------------|-------|
...
Legacy frames are no longer translated on the MCU, but this table remains for
teams maintaining backwards-compatible host tools.
